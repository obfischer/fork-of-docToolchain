...
TIP: You can specify an alternative installation directory by setting `DTC_ROOT` to a custom value.
Please keep in mind the `DTC_ROOT` must be permanently specified, otherwise `dtcw` will not be able to find your installation.
...
. Open the docToolchain wrapper script (`dtcw`, respective `dtcw.ps1` and `dtcw.bat`) in your favorite text editor and look for the line with `DTC_VERSION` which should be located near the start of the file:
[source, bash]
----
